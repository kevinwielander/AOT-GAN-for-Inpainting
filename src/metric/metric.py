...
from .inception import InceptionV3  # Adjusted import statement
...
def fid(reals, fakes, num_worker=8, real_fid_path=None):
    ...
    model = InceptionV3([block_idx])  # Removed .cuda()
...
def get_activations(images, model, batch_size=64, dims=2048, cuda=False, verbose=False):
    ...
    for i in tqdm(range(n_batches), desc="calculate activations"):
        if verbose:
            print(f"Propagating batch {i + 1}/{n_batches}", end="", flush=True)
        start = i * batch_size
